...
Translate these CSS custom properties directly into whatever theming system is used (Tailwind config, CSS variables, etc.) — do not substitute similar-looking colors.
...
- **Current mockup behavior:** `event.preventDefault()` + `alert()`. This must be replaced with a real submission handler — POST to an API route, validate server-side, rate-limit, and either email the lead or store it (Supabase/Neon table: `leads` or similar).
...
Original approved static mockup: `pecuniastudios-mockup.html` — treat as the pixel-accurate visual reference for spacing, hover states, and responsive behavior not fully captured in prose above.
